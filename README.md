...
۱. فایل PDF را در گیت‌هاب آپلود کن:
   ...
   - برو توی پوشه‌ی `files/books/11` → **Add file → Upload files** → فایل PDF را بکش و رها کن، اسمش را `book.pdf` بگذار → Commit
...
- `glyph` یک حرف/نماد که روی جلد رنگی نشان داده می‌شود
- `id` باید با هیچ کتاب دیگری یکی نباشد

۳. Commit کن. همین — Cloudflare خودش صفحه‌ی اختصاصی، سئو، و sitemap را می‌سازد.
